...
def test_copy(tmp_path):
    ...
    with open(tmp_path / "from.txt", "rb") as from_file, open(
        ...
        file.copy(from_file, to_file, len(test_contents))
    with open(tmp_path / "to.txt", "rb") as to_file:
        assert to_file.read() == test_contents


def test_copy_partial(tmp_path):
    with open(tmp_path / "from.txt", "wb") as from_file:
        from_file.write(b"test contents")
    with open(tmp_path / "from.txt", "rb") as from_file, open(
        tmp_path / "to.txt", "wb"
    ) as to_file:
        file.copy(from_file, to_file, 4)
    with open(tmp_path / "to.txt", "rb") as to_file:
        assert to_file.read() == b"test"


def test_copy_seek(tmp_path):
    with open(tmp_path / "from.txt", "wb") as from_file:
...
def test_copy_combined(tmp_path, from_offset, to_offset, copy_length):
    ...
    with open(tmp_path / "from.txt", "rb") as from_file, open(
        ...
        file.copy(from_file, to_file, copy_length)
    with open(tmp_path / "to.txt", "rb") as to_file:
        assert (
            to_file.read()
            == b"\x00" * to_offset
